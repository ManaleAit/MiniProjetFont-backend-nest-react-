...
const validationSchema = Yup.object({
  massar: Yup.string()
    .required("le massar est obligatoire")
    .min(4, "le massar doit comporter au moins quatre caractères "),
  cin : Yup.string()
  ...
  return (
    <React.Fragment>
      ...
      >
        {({ errors, touched }) => (
          <Form>
            <Grid container spamassarg={3}>
              <Grid item xs={12} sm={6}>
                <Field
                  required
                  id="massar"
                  name="massar"
                  label="massar (Code d'Identité National)"
                  fullWidth
                  as={TextField}
                  disabled = {true}
                  //value={formData.massar}
                  helperText={touched.massar ? errors.massar : ""}
                  error={touched.massar && Boolean(errors.massar)}
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <Field
                  ...
                  id="cin"
                  name="cin"
                  label="Code Massar OU cin (Code Nationnal de l'étudiant)"
                  fullWidth
                  as={TextField}
                  disabled = {true}
                  autoComplete="lname"
